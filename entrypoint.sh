...
cp -R "$SRC_DIR"/* "$CLONE_DIR/$INITIATIVE_NAME/$SYSTEM/$SYSTEM_NAME" || (rm -Rf "$CLONE_DIR" && exit 1)
cp readme.adoc "$CLONE_DIR/$INITIATIVE_NAME/$SYSTEM/$SYSTEM_NAME.adoc" || (rm -Rf "$CLONE_DIR" && exit 1)
